 ...
 */
 let all_cards = ["fa-diamond", "fa-paper-plane-o", "fa-anchor", "fa-bolt", "fa-cube", "fa-anchor", "fa-leaf", "fa-bicycle", "fa-diamond", "fa-bomb", "fa-leaf", "fa-bomb", "fa-bolt", "fa-bicycle", "fa-paper-plane-o", "fa-cube"];
 let first_card;
 let score_counter = 0;
 let time_keeper = new Date();
 let match_counter = 0;
 let stars_counter = 3;
 let lock_game = false;
 let time_in_seconds, time_in_minutes;
...
// Game Timer
 let game_time = setInterval(function() {
   if (game_ended == false ) {
     ...
     let time_difference = Math.floor((update_time-time_keeper)/1000);
...
function gameTracker(clickedCard) {
  ...
  if (first_card) { //Checking that first card is already open and user clicking on the second card in pair
    ...
    let subCard = first_card.getElementsByTagName('I');
    let subCard2 = clickedCard.getElementsByTagName('I');
...
function restartGame() {
  ...
  shuffle(all_cards);
  let blah2 = document.querySelectorAll('.card')
  let blah = document.querySelectorAll('.card .fa')
  for (let i = 0; i < all_cards.length; i++) {
